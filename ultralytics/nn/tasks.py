            ...
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, DWConv)) and hasattr(m, "bn"):
                    if isinstance(m, Conv2):
                        m.fuse_convs()
                    if isinstance(m.bn, ((nn.BatchNorm2d), (nn.GroupNorm))):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {BottleneckCSP, C1, C2, C2fAttn, C2f, C3, C3TR, C3Ghost, C3x, RepC3}:
                args.insert(2, n)  # number of repeats
                n = 1
            # Check if module should handle norm_type
            if issubclass(m, Conv):
                args.append(norm_type)
            elif m is AIFI:
                args = [ch[f], *args]
            elif m in {HGStem, HGBlock}:
                c1, cm, c2 = ch[f], args[0], args[1]
        ...
        if m in {Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            if issubclass(m, Conv):
                args.append(norm_type)  # Append norm_type for Conv-derived classes

            # Instantiate modules
            m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)
        else:
            m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # For other types of modules
